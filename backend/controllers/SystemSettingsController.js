...
const getDashboardLogo = (req, res) => {
    try {
        ...
        const dashboardLogoFile = files.find(file => file.startsWith('dashboardLogo'));

        if (!dashboardLogoFile) {
            return res.status(404).json({ success: false, error: 'Dashboard logo not found' });
        }
        ...
        return res.status(500).json({ success: false, error: 'Failed to get dashboardLogo image' });
    }
};
